...
export default class Client {
    ...
    get<T extends ShopifyType<string>>(type: ShopifyTypeStr, handle: string): Promise<T> {
        ...
        if(cache_result) return new Promise((res) => (res(cache_result)));

        // Otherwise, request it from Shopify
        return fetch(this.url_prefix + this._resolve_path(type, handle))
            .then((res) => (res.json()))
            .then((json) => this._processResponse.call(this, type, handle, json))
            .catch(defaultErrorHandler);
    }
     ...
     */
    _processResponse(type: ShopifyTypeStr, handle: string, json: ShopifyInstanceWrapper<ShopifyType<string>>) {
        ...
        return this.getCollectionProducts(handle)
            .then((products) => mergeCollectionItems.call(this, products));
    }
}
