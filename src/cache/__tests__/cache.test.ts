...
import { indexSingleElement, Cache } from "../index";
...
describe('writeCache()', () => {
    ...
    it('creates new references for each', () => {
        ...
        expect(newCache._cache.handles[ShopifyTypeEnum.Collection] !== 
            cache._cache.handles[ShopifyTypeEnum.Collection]).toBeTruthy();
        expect(newCache._cache.handles[ShopifyTypeEnum.Product] !== 
            cache._cache.handles[ShopifyTypeEnum.Product]).toBeTruthy();
        expect(newCache._cache.handles[ShopifyTypeEnum.Page] !== 
            cache._cache.handles[ShopifyTypeEnum.Page]).toBeTruthy();
    });
});
